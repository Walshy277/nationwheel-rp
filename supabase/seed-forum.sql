...
('War Room','Military strategy, wars, and battle reports.','war-room','⚔',9,(select id from forum_categories where slug='in-character'),'public'),
...
('Embassies','Embassy threads and diplomatic offices.','embassies','🏛',15,(select id from forum_categories where slug='nation-roleplay'),'public'),
('Treaties','Treaty drafts, signings, and treaty archives.','treaties','🖋',16,(select id from forum_categories where slug='nation-roleplay'),'public'),
('Military Actions','Military movement and conflict actions.','military-actions','🛡',17,(select id from forum_categories where slug='nation-roleplay'),'members'),
...
('Season Archives','Completed seasons and historical records.','season-archives','📦',19,(select id from forum_categories where slug='archives'),'archived'),
('Staff Room','Private staff coordination.','staff-room','⚙',22,(select id from forum_categories where slug='staff-area'),'staff'),
('Disciplinary Records','Private warnings and disciplinary notes.','disciplinary-records','⚖',27,(select id from forum_categories where slug='staff-area'),'staff')
on conflict (slug) do update set
  name = excluded.name,
  description = excluded.description,
  icon = excluded.icon,
